...
<h1 class="style3"><font face=verdana><img src="images/alterar.png" width="48" height="48"> Cadastrar C&oacute;digos Conclus&atilde;o:</font></h1>
...

{
 ...
 $busca_cod="select * from codigo_conc where codigo = '".$codigo."';";
...

 {
  ...
  $cad_cod="insert into codigo_conc values ('','".$codigo."','".$descricao."');";
...

  {
   ...
   $busca_cod2="select * from codigo_conc where codigo = '".$codigo."';";
   $res_busca_cod2=mysql_query($busca_cod2,$conn);
   $campo_cod2=mysql_fetch_array($res_busca_cod2);
   $last_id = mysql_insert_id();
 echo "<script>alert('Código de Conclusão Nº $last_id Cadastrado.');window.location='cad_cod_conc.php?id=$campo_cod2[id]';</script>";
...

 {
  ...
  echo "<script>alert('O código: $codigo de Conclusão já está cadastrado.');</script>";
