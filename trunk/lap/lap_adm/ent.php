...
if(($id_pac!='')and($id_med!='')and($data_entrada!='')and($data_previsao!='')and($id_ex!='')and($material!='')and($convenio!=''))
{
...
if(($_POST['id_ex']==$campo_tipos[id_ex])or($_POST['id_ex']==$campo_tipos2[id_ex])or($_POST['id_ex']==$campo_tipos3[id_ex])or($_POST['id_ex']==$campo_tipos4[id_ex])){


//Buscando o tipo de exame sendo Citologia, Citologia oncotica, Colposcopia e Descarga papilar 
$busca_id_exa="select * from exame where tipo = '".$campo_tipos[id_ex]."' or tipo = '".$campo_tipos2[id_ex]."' or tipo = '".$campo_tipos3[id_ex]."' or tipo = '".$campo_tipos4[id_ex]."' order by id DESC limit 0,1;";
...
$id_exame = $campo_id_exa['id_exame']+1;
}
}
//Até aqui

//TIPO DE EXAME SENDO : Demais tipos
if(($_POST['id_ex']!=$campo_tipos[id_ex])and($_POST['id_ex']!=$campo_tipos2[id_ex])and($_POST['id_ex']!=$campo_tipos3[id_ex])and($_POST['id_ex']!=$campo_tipos4[id_ex])){


//Buscando o tipo de exame sendo Citologia, Citologia oncotica, Colposcopia e Descarga papilar 
$busca_id_exa="select * from exame where tipo != '".$campo_tipos[id_ex]."' and tipo != '".$campo_tipos2[id_ex]."' and tipo != '".$campo_tipos3[id_ex]."' and tipo != '".$campo_tipos4[id_ex]."' order by id DESC limit 0,1;";
$res_busca_id_exa=mysql_query($busca_id_exa,$conn);
$campo_id_exa=mysql_fetch_array($res_busca_id_exa);
...
##Gerar o ID do exame de acordo com os cadastrados Até aqui##

    $cad_ex="insert into exame values ('','".$id_exame."','".$id_pac."','".$id_med."','','".$convenio."','".$campo_cod2['tipo']."','".$codigo_procedimento."','','','".$ex_status_id."','".$data_entrada."','".$data_previsao."','".$data_saida."','".$id_ex."','".$material."','".$lab."','".$macroscopia."','','".$microscopia."','','".$conclusao."','".$valor."','".$obs_entrada."','".$obs_saida."','','','','','','".$usuario_autenticado."','','','g');";
...

  {
   ...
   //echo $last_id;
    ...
    echo "<script>alert('Exame Cadastrado com Sucesso. Exame Nº  $last_id');window.open('reimp.php?id=$last_id');</script>";
...
<?
$busca_paci="select * from paciente where id = '".$_POST[id_pac]."';";
...
<tr>
  ...
  <td width="367"><select name="id_pac" class="caixa" id="id_pac" onChange="submitar()">
   ...
   printf("<option value='$_POST[id_pac]'>$campo_paci[nome]");
    } else { ?>
	<?
...

{
...

 {
  ...
  printf("<option value='$campo_conv[id]'>$campo_conv[nome]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$campo_conv[convenio];&nbsp;&nbsp;&nbsp;(Mae:$campo_conv[nome_mae])");
...
<tr> <td>Previs&atilde;o de Saida: </td>
